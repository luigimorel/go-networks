...
func simplePost()  {
	res, err := http.Post("http://localhost:7070","application/x-www-form-urlencoded", strings.NewReader("name=Radek&surname=Sohlich"))
	if err != nil {
		panic(err)
	}
...
func useRequest()  {
	...
	res.Body.Close()
	fmt.Println("Respose from server" + string(data)) 
}